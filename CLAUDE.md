...
cot-injection-monitoring/          ← repo root (committed to git)
  ...
  src/
    infra/                         ← cross-cutting, env-aware (local + Colab)
      ...
      llm_client.py                ← LLMClient (OpenRouter backend + MockBackend for offline tests)
    data/
    ...
    attacks/
      ...
      generate.py                  ← [Phase 3] generator-LLM calls for B/C/D via OpenRouter (layer 2)
...
# For judge/generator calls (OpenRouter):
...
- `src/infra/llm_client.py` — `LLMClient` (OpenRouter + MockBackend); `from_env()`
- `config/experiment.yaml` — all §6.6.3 knobs + `samples_per_condition`
- `config/models.yaml` — subject/judge/generator configs (OpenRouter model IDs)
...
**Decisions & rationale:**
- **OpenRouter** for all judge/monitor/generator calls (one key, swap models without code changes).
...
**Phase goal:** Families A/B/C/D templates, OpenRouter generator, length control ±12%, per-family validators, C built per-B (matched pairs). All model-agnostic cache Layer 2.
...
- `src/attacks/generate.py` — `generate_family_a()` (template, free); `generate_prefill()` (B/C/D via OpenRouter; failures NOT cached); `generate_all_prefills()` (two-pass B/D first then C; dedup; batch stats)
